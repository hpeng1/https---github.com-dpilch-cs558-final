...
int main () {
	...
	// UNIT TEST 1: A BIG ONE!!
	{
		...
		//MyDB_PageHandle pinnedPage1 = myMgr.getPinnedPage (table1, 1);
		//MyDB_PageHandle hot = myMgr.getPinnedPage (table1, 0);
		/* Hao's test space
        MyDB_PageHandle hot = myMgr.getPinnedPage (table1, 10);
        MyDB_PageHandle hot1 = myMgr.getPinnedPage (table1, 0);
        MyDB_PageHandle hot2 = myMgr.getPinnedPage (table1, 8);
        MyDB_PageHandle hot3 = myMgr.getPinnedPage (table1, 1);
        Hao's test space*/
        
        cout << "I got the handle of page " << pinnedPage->page_id<<endl;

        //write a few bytes to the page
		char *bytes = (char *) pinnedPage->getBytes();
		cout<<"i can access the content of the page: "<<bytes<<endl;

		writeNums (bytes, 64, 0);
		cout<<"i can write to the page's content!"<<endl;
		pinnedPage->show_content(64);
		cout<<""<<endl;
        //mark the page dirty
		...
		for (int i = 1; i < 20; i++) {
			...
			temp->show_content(64);
			cout<<""<<endl;
			temp->wroteBytes ();
			myHandles.push_back (temp);
		}

		//cout<<"testing content:"<<endl;
		//myHandles[2]->show_content(64);

		MyDB_PageHandle hot = myMgr.getPinnedPage();
